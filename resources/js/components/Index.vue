      ...
      <div class="container">
        <div class="row">
          <div class="col-xl-9">
            ...
            <div v-for="(products, menucat_slug) in catProducts" v-bind:key="products.id">
              <h2 :id="menucat_slug">{{ products['name'] }}</h2>
              <div class="main__filter" v-if="menucat_slug == 'pizzas'">
                <div class="main__filter__inner">
                  <button class="main__filter__button"
                          @click="categories = []"
                          v-bind:class="{active: categories.length === 0}"
                  >Все</button>
                  <label class="checkbox"
                         v-for="item in filters"
                         v-bind:key="item.name">
                    <input type="checkbox" :value="item.id" v-model="categories" />
              ...
              <div class="row main__menu" v-if="menucat_slug === 'pizzas'">
                <ProductCard v-for="product in filterData" v-bind:key="product.id" v-bind:item="product"></ProductCard>
              </div>
              <div class="row main__menu" v-if="menucat_slug !== 'pizzas'">
            ...
            <div class="main__cart">
              <div class="main__cart__header">
                <div class="main__cart__title">
                  ...
                  <h3>Корзина <span>({{ cartItemsCount }})</span></h3>
                </div>
              ...
              <div class="main__cart__empty" v-if="cartItems.length === 0">
                <img src="~@/assets/img/main/cart-empty.svg">
              ...
              <router-link to="/cart" class="button button-cart-arrow"  v-bind:class="{ disabled: cartItems.length === 0 }">Оформить заказ</router-link>
            </div>
          </div>
        </div>
      </div>
...
import Seo from '@/components/parts/Seo.vue'
...
export default {
  ...
  data() {
    return {
      ...
      images: [
        {
          id: 1,
          url: require('@/assets/img/main/slider.jpg'),
          text1: 'СКИДКА',
          text2: '-20%',
          text3: 'на самовывоз'
        },
      ...
      filters: [
        {
          id: 1,
          url: require('@/assets/img/common/veg.svg'),
          name: 'VEG',
        },
        {
          id: 2,
          url: require('@/assets/img/common/hot.svg'),
          name: 'Острые',
        },
        {
          id: 3,
  ...
  methods: {
    ...
    ...mapMutations(['updateCartTotal']),
  },
  computed: {
    catProducts() {
      return this.products.reduce((p,c) => {
        const menucat_name = c.menucat_name;
        const menucat_slug = c.menucat_slug;
        p[menucat_slug] = p[menucat_slug] || [];
        p[menucat_slug]['name'] = menucat_name;
        p[menucat_slug].push(c);
        return p;
      }, {});
    },
    filterData() {
      ...
      if (this.categories.length) {
        data = this.products.filter(x => this.categories.indexOf(x.pizzacat_id) + '' != -1)
      } else {
        // иначе отдаем все данные из массива
        data = this.catProducts['pizzas']
      }
      return data
    },
  ...
  .checkbox
    ...
    input:checked + .checkbox__text
      .main__filter__button
        ...
        box-shadow: 0px 5px 8px rgba(34, 39, 41, 0.05)
...
.main__cart
  ...
  &__header
    ...
    background: $main
    box-shadow: inset 0px 2px 2px #8FEAB1, inset 0px -2px 2px rgba(0, 99, 28, 0.3)
    display: flex
